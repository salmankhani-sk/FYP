...
import { useRouter } from "next/navigation"; // For redirect on 401
...
const RecipeGenerator = () => {
    ...
    const fetchRecipe = async (dish: string) => {
        ...
        try {
            ...
            const data = await response.json();
            setRecipe(data.recipe); // Set the fetched recipe
            fetchVideos(dish); // Fetch videos after recipe is successfully generated
        } catch (error: any) {
            console.error("Error fetching recipe:", error);
            setRecipe(`Error: ${error.message}`); // Display error to user
        } finally {
            setLoading(false);
        }
    };
    ...
    const fetchVideos = async (dish: string) => {
        try {
            ...
            const data = await response.json();
            setVideos(data.videos); // Set the fetched videos
        } catch (error) {
